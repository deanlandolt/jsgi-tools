...
//
// Ported to javascript by J. Chris Anderson <jchris@apache.org>, targeting the Spidermonkey runtime.
// Comments are mostly excerpted from the original.
//
// Ported by Dean Landolt <dean@deanlandolt.com> to CommonJS
//
// Updated to use Securable Modules
// TODO change tests to use CommonJS assertions
// Changed to crock-style formatting
...
function parseRanges(ranges) {
    ...
    for (var i = 0; i < rangeParts.length; i++) {
        parsedRanges.push(mime.parseMediaRange(rangeParts[i]))
    };
    return parsedRanges;
};
...
mime.parse.mimeType = function(mimeType) {
...
mime.parse.mediaRange = function(range) {
    var q, parsedType = mime.parse.mimeType(range);
    if (!parsedType[2]["q"]) {
        parsedType[2]["q"] = "1";
    }
    else {
...
// parsed by mime.parse.mediaRange(). Returns an array of
// the fitness value and the value of the 'q' quality
// parameter of the best match, or (-1, 0) if no match
// was found. Just as for qualityParsed(), 'parsed_ranges'
// must be a list of parsed media ranges.
mime.fitnessAndQualityParsed = function(mimeType, parsedRanges) {
    var bestFitness = -1, bestFitQ = 0, target = mime.parse.mediaRange(mimeType);
...
// Find the best match for a given mime-type against
// a list of media_ranges that have already been
// parsed by mime.parse.mediaRange(). Returns the
// 'q' quality parameter of the best match, 0 if no
// match was found. This function bahaves the same as quality()
// except that 'parsedRanges' must be a list of
// parsed media ranges.
mime.qualityParsed = function(mimeType, parsedRanges) {
    return mime.fitnessAndQualityParsed(mimeType, parsedRanges)[1];
},
...
// 0.7
mime.quality = function(mimeType, ranges) {
    return mime.qualityParsed(mimeType, parseRanges(ranges));
},
...
// "text/xml"
mime.bestMatch = function(supported, header) {
    var parsedHeader = parseRanges(header);
    var weighted = [];
    for (var i=0; i < supported.length; i++) {
        weighted.push([mime.fitnessAndQualityParsed(supported[i], parsedHeader), supported[i]])
    };
    weighted.sort();
